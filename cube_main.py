...
fun = False
pixel_set = set()
d_angle = math.radians(ROTATION_SPEED)
...
for cube_x in 1, 0:
    for cube_y in 1, 0:
        for cube_z in 1, 0:
            cube_coordinates.append((-100 + cube_x * 200, -100 + cube_y * 200, -100 + cube_z * 200))
...
while True:
    ...
    for index, _ in enumerate(cube_coordinates):
        cube_point = cube_coordinates[index]
        ###Z-AXIS
        if keys[pygame.K_d]:
            cube_point = z_matrix_transformation(cube_point,d_angle*(-1))
        if keys[pygame.K_a]:
            cube_point = z_matrix_transformation(cube_point,d_angle)
        ###X-AXIS
        if keys[pygame.K_w]:
            cube_point = x_matrix_transformation(cube_point,d_angle*(-1))
        if keys[pygame.K_s]:
            cube_point = x_matrix_transformation(cube_point,d_angle)
        ###Y-AXIS
        if keys[pygame.K_q]:
            cube_point = y_matrix_transformation(cube_point,d_angle*(-1))
        if keys[pygame.K_e]:
            cube_point = y_matrix_transformation(cube_point,d_angle)
